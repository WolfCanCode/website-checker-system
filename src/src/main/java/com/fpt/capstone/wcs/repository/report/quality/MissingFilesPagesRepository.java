...
public interface MissingFilesPagesRepository extends JpaRepository<MissingFileReport,Long> {
    ...
    @Query(value = "SELECT DISTINCT * " +
            ...
            "ORDER BY a.created_time DESC " +
            "LIMIT 5", nativeQuery = true)
    List<MissingFileReport> findAllGroupByCreatedTimeAndPageOption(@Param("id") long id, @Param("pageId") long pageId);

    List<MissingFileReport> findALlByCreatedTime(Date time);
}
