...
public interface CookieRepository extends JpaRepository<CookieReport,Long> {
    ...
    @Query(value = "SELECT DISTINCT * " +
            "FROM cookie_report a, page_option p, website w, user u, website_user wu " +
            "WHERE a.del_flag = 0 " +
    ...
    List<CookieReport> findAllGroupByCreatedTime(@Param("id") long id);
    ...
    @Query(value = "SELECT DISTINCT * " +
            ...
            "ORDER BY a.created_time DESC " +
            "LIMIT 5", nativeQuery = true)
    List<CookieReport> findAllGroupByCreatedTimeAndPageOption(@Param("id") long id, @Param("pageId") long pageId);

    List<CookieReport> findALlByCreatedTime(Date time);
}
