...
public interface ServerBehaviorRepository extends JpaRepository<ServerBehaviorReport, Long> {
    ...
    @Query(value = "SELECT DISTINCT * " +
            "FROM server_behavior_report a, page_option p, website w, user u, website_user wu " +
            "WHERE a.del_flag = 0 " +
    ...
    List<ServerBehaviorReport> findAllGroupByCreatedTime(@Param("id") long id);
    ...
    @Query(value = "SELECT DISTINCT * " +
            ...
            "ORDER BY a.created_time DESC " +
            "LIMIT 5", nativeQuery = true)
    List<ServerBehaviorReport> findAllGroupByCreatedTimeAndPageOption(@Param("id") long id, @Param("pageId") long pageId);

    List<ServerBehaviorReport> findALlByCreatedTime(Date time);
}
