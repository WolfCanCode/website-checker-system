...
public interface ProhibitedContentRepository extends JpaRepository<ProhibitedContentReport,Long> {
    ...
    @Query(value = "SELECT DISTINCT * " +
            ...
            "ORDER BY a.created_time DESC " +
            "LIMIT 5", nativeQuery = true)
    List<ProhibitedContentReport> findAllGroupByCreatedTimeAndPageOption(@Param("id") long id, @Param("pageId") long pageId);

    List<ProhibitedContentReport> findALlByCreatedTime(Date time);
}