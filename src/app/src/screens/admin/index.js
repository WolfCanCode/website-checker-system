        ...
        for (let i = 0; i < menuList.length; i++) {
            if (menuList[i].items === null) {
                if (menuList[i].to === this.props.location.pathname) {
                    this.setState({ titleHeader: menuList[i].name, altHeader: menuList[i].alt });
                    check = true;
                }
            } else {
                for (let j = 0; j < menuList[i].items.length; j++) {
                    if (menuList[i].items[j].to === this.props.location.pathname) {
                        this.setState({ titleHeader: menuList[i].items[j].name, altHeader: menuList[i].items[j].alt, imageSrc: menuList[i].items[j].image });
                        check = true;
                    }
                }
            }
