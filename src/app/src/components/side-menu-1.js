...
export default class SideMenu extends Component {
    ...
    async _mapItem() {
        this.listMenuItem = menu.map((item, index) => {
            if (item.items === null) {
                return (
                    <Menu.Item key={index} as={Link} to={item.to} active={this.state.menuActive === item.key ? true : false} onClick={() => this._doActiveChange(item)}>
                        {item.name}
                    </Menu.Item>);
            } else {
                var childItem = item.items.map((itemC, indexC) => {
                    return (
                        <Menu.Item key={indexC} as={Link} to={itemC.to} active={this.state.menuActive === itemC.key ? true : false} onClick={() => this._doActiveChange(itemC)}>
                            {itemC.name}
                        </Menu.Item>
                    );
                });
